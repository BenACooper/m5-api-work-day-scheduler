...
function loadDescriptionsFromStorage() {
  ...
  // Iterate through the keys and update textarea elements
  for (var i = 0; i < keys.length; i++) {
    var key = keys[i];
    
    // Check if the key matches the format of your timeBlockIds
    if (key.startsWith('hour-')) {
      var savedDescription = localStorage.getItem(key);
      $("#" + key + " .description").val(savedDescription);
    }
  }
}
